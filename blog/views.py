...
from user_auth.sms import product_registered_customer, product_registered_seller
...
def callback_gateway_shop(request):
    ...
    if bank_record.is_success:
        ...
        wish_list = wishlist.objects.filter(buyer=request.user).filter(paid=False)
        for x in wish_list:
            x.product_choice.count = x.product_choice.count-1
            saler = User.objects.get(shop=myshop.objects.get(products=x.product))
            product_registered_seller(saler.mobile, 'shopeton.ir/sold/')
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        context['all_shop'] = myshop.objects.filter(title__contains=search)
        context['all_product'] = Product.objects.filter(name__contains=search)
        context.update(context_sample)
